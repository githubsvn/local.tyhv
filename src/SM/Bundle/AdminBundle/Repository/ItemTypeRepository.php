...
class ItemTypeRepository extends EntityRepository
{
    ...
    {
        $repositor = $this->_em->getRepository('SMAdminBundle:ItemType');
        //get $tree childrens
        $children = $this->_em->getRepository('SMAdminBundle:ItemType')
    ...
    /**
     * @param \SM\Bundle\AdminBundle\Entity\ItemType $page the page
     */
    public function moveUp(\SM\Bundle\AdminBundle\Entity\ItemType $page)
    {
        //get the area upper
        ...
        $repositor = $entityManager->getRepository('SMAdminBundle:ItemType');
     ...
     * @param \SM\Bundle\AdminBundle\Entity\ItemType $page the page
     */
    public function moveDown(\SM\Bundle\AdminBundle\Entity\ItemType $page)
    {
        //get the area upper
        $entityManager = $this->_em;
        $pageArray = array(
        ...
        if (is_array($entities) && count($entities) > 0) {
            foreach ($entities as $obj) {
                if (is_object($defaultLanguage) && is_object($obj)) {
                    ...
                    $std = new \stdClass();
					$objLang = $obj->getCurrentLanguage();
					if (is_object($objLang)) {
						$std->name = $objLang->getTreeName();
						$std->id = $obj->getId();
						$options[] = $std;
					}
                }
    ...
    public function getAllChildrenIds($parentId = '')
    {
        ...
        $ids[] = intval($parentId);
        if (!empty($parentId)) {
            $this->getSubChildBranch($parentId, $ids);
        }
    ...
    /**
     * Get all child id from parent id
     */
    public function getSubChildBranch($parentId, &$ids)
    {
        $rst = $this->getChildrenArray($parentId);
        if (is_array($rst) && count($rst) > 0) {
            foreach ($rst as $item) {
                if (!empty($item['id'])) {
                    $ids[] = $item['id'];
                    $rst[] = $this->getSubChildBranch($item['id'], $ids);
                }
            }
        }
