...
class MediaController extends Controller
{
    ...
    public function indexAction($page, $lang)
    {
        ...
        $criteria = array();
        if (!empty($type)) {
            $criteria['category'] = $type;
        }
        $entities = $rep->getList($perPage, ($page - 1) * $perPage, $criteria);

        //get upload dir
        $thumbUploadPath = $this->thumbPath;
        
        //Set data for company type on form search
        $repMediType = $this->getDoctrine()->getRepository('SMAdminBundle:MediaCategory');
        $optMediaTypes = $repMediType->getList();
        foreach ($optMediaTypes as $oMediaType) {
            $oMediaType->setLanguage($currentLanguage);
        }
